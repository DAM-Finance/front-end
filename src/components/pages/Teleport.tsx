          ...
          {appStore.walletProvider?.connected ? (
            <div className="flex flex-col gap-3">
              <button
                ...
                disabled={destinationNetwork.chainId === '0x504' || isBelowZero() || isAboveBalance()}
              >
                <img src={utils.getImageSrc('teleport.svg')} alt="teleport" />
                {isAboveBalance() ? 'Insufficient balance' : destinationNetwork.chainId === '0x504' ? 'Teleport temporary disabled' : 'Teleport'}
              </button>
